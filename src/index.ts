...
// Establish connection to tf2-rcon websocket.
  const connectTf2rconWebsocket = () => {
    ...
    if (tf2rconWs !== null) {
      tf2rconWs.addEventListener('message', (data) => {
      ...
      tf2rconWs.addEventListener('close', (event) => {
      ...
      tf2rconWs.addEventListener('error', (event) => {
        console.log(`err: ${JSON.stringify(event)}`); // %TODO
        // console.error('[main.ts] WebSocket error:', err.message); // %TODO
        if (tf2rconWs !== null) {
      ...
      file.on('finish', () => {
        file.close(() => {
          ...
          if (fileHash === expectedSHA1) {
            ...
            fs.unlink(dest, () => {
              console.log(`Hash mismatch! Dest: ${dest}`)
            }); // %TODO, Delete the file on hash mismatch
